...
public class XGBoostJsonParser implements LtrRankerParser {
    ...
    private static class SplitParserState {
        ...
        private boolean isCategoricalSplit;
            ...
            if (split.isSplit()) {
                ...
                if (!split.splitHasValidChildren()) {
                    throw new ParsingException(parser.getTokenLocation(), "Split structure is invalid, yes, no and/or" +
                            " missing branches does not point to the proper children. " + split.nodeId + " children: " +
                            split.children + " left node" + split.leftNodeId + " right node" + split.rightNodeId);
                }
        ...
        boolean splitHasAllFields() {
            return nodeId != null && (numericalThreshold != null || categoricalThreshold != null) && split != null
                    && leftNodeId != null && rightNodeId != null && depth != null
        ...
        boolean splitHasValidChildren() {
            return children.size() == 2 &&
                    children.stream().anyMatch(a -> a.nodeId.equals(leftNodeId)) &&
                    children.stream().anyMatch(a -> a.nodeId.equals(rightNodeId));
        }
        boolean isSplit() {
            return leaf == null;
        }
            ...
            if (isSplit()) {
                if (isCategoricalSplit()) {
                    return new NaiveAdditiveDecisionTree.Split
                            (children.stream()
                                    .filter(a -> a.nodeId.equals(leftNodeId))
                                    .findFirst()
                                    .get()
                                    .toNode(set),
                                    children.stream()
                                            .filter(a -> a.nodeId.equals(rightNodeId))
                                            .findFirst()
                                            .get()
                                            .toNode(set),
